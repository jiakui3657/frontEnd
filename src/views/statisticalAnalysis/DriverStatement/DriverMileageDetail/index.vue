...
import { MileageDetail } from '@/api/statisticalAnalysis/driver' // Excel导出
...
export default {
  ...
  methods: {
    ...
    getSpeedList() { // 分页获取智能预警列表
      ...
      MileageDetail(this.listQuery).then(res => {
        if (res.code === 0) {
          this.listLoading = false
          res.data.rows.forEach((item, i) => {
            item.totalMileageCount.forEach((it, j) => {
              if (i === 0) {
                this.columns.push({ key: it.statDate, title: it.statDate, width: '100' })
              }
              this.$set(this.mileageData, it.statDate, it.totalMileage)
