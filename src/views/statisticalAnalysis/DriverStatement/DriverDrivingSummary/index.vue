...
import { Driving, DrivingChart } from '@/api/statisticalAnalysis/driver' // Excel导出
...
export default {
  ...
  methods: {
    ...
    getSpeedList() { // 分页获取智能预警列表
      ...
      Driving(this.listQuery).then(res => {
        if (res.code === 0) {
    ...
    gettotalMileage() { // 分页获取智能预警列表
      ...
      DrivingChart(this.listQuery).then(res => {
        if (res.code === 0) {
          this.listLoading = false
          res.data.forEach(item => {
            this.lineChartData.expectedData.push(item.date)
            this.lineChartData.actualData.push(item.statusCount)
          })
