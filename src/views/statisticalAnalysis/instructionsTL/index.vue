    ...
    <el-container>
      <el-form
        ...
        <el-form-item label="指令类型">
          <el-select
            v-model.trim="listQuery.commandType"
            placeholder="全部"
            style="width:120px;"
            size="small">
            <el-option
...
<script>
import { getInstructionsTL, getCommand } from '@/api/statisticalAnalysis/instructionsTL'
...
export default {
  ...
  created() {
    this.getList()
    getCommand().then(res => {
      this.options = res.data.command.map(val => {
        return {
          value: val.name,
          label: val.name
        }
  ...
  methods: {
    getList() {
      getInstructionsTL(this.listQuery).then(res => {
