...
import { enterpriseMileageSummary, enterpriseMileageExcel } from '@/api/statistics/statistics'
...
export default {
  ...
  methods: {
    ...
    getSpeedList() { // 分页获取智能预警列表
      ...
      enterpriseMileageSummary(this.listQuery).then(res => {
        console.log(res)
        if (res.code === 0) {
    ...
    alarmexport() {
      enterpriseMileageExcel(this.listQuery).then(result => {
    ...
    downloads(data) {
      ...
      link.href = url
      link.setAttribute('download', '企业里程汇总.xls')
