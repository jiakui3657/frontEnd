<template>
    <div class="app-container">
      ...
      <el-main>
        <el-table :data="data" :columns="columns" v-loading.body="listLoading" border stripe style="width:100%; border-radius: 5px; box-shadow: 0 0 10px #ccc" :header-cell-style="{ 'background-color': '#5F7ABF', 'color': '#fff' }">
...
import { summaryInfo, alarmExcel } from '@/api/statistics/statistics' // Excel导出
...
export default {
  ...
  methods: {
    ...
    getSpeedList() { // 分页获取智能预警列表
      ...
      summaryInfo(this.listQuery).then(res => {
        if (res.code === 0) {
          this.listLoading = false
          res.data.rows.forEach((item, i) => {
            item.total.forEach((it, j) => {
              if (i === 0) {
                this.columns.push({ key: it.dateTime, title: it.dateTime, width: '100' })
              }
              this.$set(this.mileageData, it.dateTime, it.runMileage)
