    ...
    <el-container>
      <el-form
        ...
        <el-form-item label="查询时间">
          <el-date-picker
            v-model.trim="listQuery.startEnd"
            type="datetimerange"
        ...
        <el-form-item label="操作类型">
          <el-select
            v-model.trim="listQuery.operation"
            placeholder="请选择"
            style="width:120px;"
            size="small">
            <el-option
...
<script>
import { getOperationalLogbook, getOperate } from '@/api/statisticalAnalysis/operationalLogbook'
export default {
  name: 'intelligentEWP',
  data() {
    return {
  ...
  created() {
    this.getList()
    getOperate().then(res => {
      this.options = res.data.operate.map(val => {
        return {
          value: val.operation,
          label: val.operation
        }
  ...
  methods: {
    getList() {
      getOperationalLogbook(this.listQuery).then(res => {
