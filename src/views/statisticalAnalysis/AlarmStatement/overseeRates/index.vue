    ...
    <el-container>
        <el-form ref="form" :model="listQuery" label-width="80px" label-position="left" inline style="background: #fff; margin: 0 20px; padding: 20px 10px 0 20px; border-radius: 5px; box-shadow: 0 0 10px #ccc">
          ...
          <el-form-item label="所属地区">
            <el-select clearable v-model="listQuery.areaId" :loading="loading" @focus="getAreaList" placeholder="请选择" style="width: 200px">
...
import { overseeRates, overseeExcel } from '@/api/statistics/statistics'
...
export default {
  ...
  methods: {
    ...
    getSpeedList() { // 分页获取智能预警列表
      ...
      overseeRates(this.listQuery).then(res => {
        console.log(res)
        if (res.code === 0) {
    ...
    alarmexport() {
      ...
      overseeExcel(this.listQuery).then(result => {
    ...
    downloads(data) {
      ...
      link.href = url
      link.setAttribute('download', '督办处理率.xls')
