...
import { alarmHandlerList, alarmHandlerExcel } from '@/api/statistics/statistics'
...
export default {
  ...
  methods: {
    ...
    getSpeedList() { // 分页获取智能预警列表
      ...
      alarmHandlerList(this.listQuery).then(res => {
        console.log(res)
        if (res.code === 0) {
    ...
    alarmexport() {
      ...
      alarmHandlerExcel(this.listQuery).then(result => {
    ...
    downloads(data) {
      ...
      link.href = url
      link.setAttribute('download', '报警处理率.xls')
