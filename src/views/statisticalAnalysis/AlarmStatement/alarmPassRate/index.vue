...
import { startnumber, startNumberExcel } from '@/api/statistics/statistics'
...
export default {
  ...
  methods: {
    ...
    getSpeedList() { // 分页获取智能预警列表
      ...
      startnumber(this.listQuery).then(res => {
        console.log(res)
        if (res.code === 0) {
    ...
    alarmexport() {
      ...
      startNumberExcel(this.listQuery).then(result => {
    ...
    downloads(data) {
      ...
      link.href = url
      link.setAttribute('download', '报警数据合格率.xls')
