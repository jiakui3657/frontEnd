...
import { getAlarmTimeList, getAlarmTimeExcel } from '@/api/statistics/statistics'
...
export default {
  ...
  methods: {
    ...
    getSpeedList() { // 分页获取智能预警列表
      ...
      getAlarmTimeList(this.listQuery).then(res => {
        console.log(res)
        if (res.code === 0) {
    ...
    alarmexport() {
      ...
      getAlarmTimeExcel(this.listQuery).then(result => {
    ...
    downloads(data) {
      ...
      link.href = url
      link.setAttribute('download', '报警平均响应时长.xls')
